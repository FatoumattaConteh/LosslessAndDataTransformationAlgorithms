...
def multi_level_compress(data, levels=10):
    """Apply BWT+MTF+Huffman compression multiple times"""
    ...
    for i in range(levels):
        ...
        # BWT Transform
        bwt_result = bwt_transform(compressed_data)
        print(f"Characters after BWT: {len(bwt_result)}")
        
        # MTF Encoding
        mtf_result, mtf_alphabet = mtf_encode(bwt_result)
        mtf_text = ' '.join(map(str, mtf_result))
        print(f"Characters after MTF: {len(mtf_text)}")
        
        # Huffman Encoding
        mtf_chars = list(map(str, mtf_result))
        huffman_encoded, huff_codes = huffman_encode(mtf_chars)
        print(f"Bits after Huffman: {len(huffman_encoded)}")
        
        # Save metadata for this level
        metadata = {
            'huffman_codes': huff_codes,
            'mtf_alphabet': mtf_alphabet,
            'bwt_length': len(bwt_result)
        }
        all_metadata.append(metadata)
        
        # Save intermediate results
        save_text(f"compressed_level_{i+1}_bwt.txt", bwt_result)
        save_text(f"compressed_level_{i+1}_mtf.txt", mtf_text)
        save_text(f"compressed_level_{i+1}_huffman.txt", huffman_encoded)
        save_json(f"metadata_level_{i+1}.json", metadata)
...
def multi_level_decompress(compressed_data, all_metadata):
    """Decompress data that was compressed multiple times"""
    ...
    for i in range(len(all_metadata)-1, -1, -1):
        ...
        # Huffman Decoding
        mtf_chars = huffman_decode(decompressed_data, metadata['huffman_codes'])
        print(f"Recovered {len(mtf_chars)} MTF symbols")
        
        # Inverse MTF
        mtf_decoded = mtf_decode(mtf_chars, metadata['mtf_alphabet'].copy())
        bwt_lastcol = ''.join(mtf_decoded)
        print(f"Recovered BWT last column, length: {len(bwt_lastcol)}")
        
        # Inverse BWT
        decompressed_data = bwt_inverse(bwt_lastcol)
        print(f"Recovered data length: {len(decompressed_data)}")
...
def qr_vc_pipeline(data):
    ...
    # Step 4: Apply multi-level compression
    compressed_data, all_metadata = multi_level_compress(flat_text, levels=10)
    
    # Save the final compressed data for fallback
    save_text("compressed_data_fallback.txt", compressed_data)
    
    # Save all metadata
    save_json("all_metadata.json", all_metadata)
    ...
    return size, compressed_data, all_metadata

# Run the compression pipeline
share_shape, compressed_data, all_metadata = qr_vc_pipeline("Transaction ID: 12345, Amount: $90")
...
# Decompress the data
print("\n--- Starting Multi-Level Decompression ---")
decompressed_text = multi_level_decompress(compressed_data_from_file, all_metadata)
